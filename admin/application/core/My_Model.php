...
// 		print_r($this->db->last_query());die;
    ...
    function delete_image_by_parent($type, $id){
        $this->db->where( "img_parent_id", $id );
        $this->db->where( "img_type", $type );
        return $this->db->delete( "core_images" );
    }
 	/**
...
// 		var_dump( $this->custom_conds( $conds ));
...
// 		echo $this->db->last_query();
	...
	function get_all_by( $conds = array(), $limit = false, $offset = false ) {
		...
		// where clause
// 		if( $this->table_name != "bs_category"){
            // print_r($conds);
		    $this->custom_conds( $conds );
// 		}
...
//     if($conds['category_id'] != '0'){
    
//      $this->db->where_not_in( 'bs_items.category_id', $conds['category_id'] ); 
//     }

//   }   
//   }
  if ( isset( $conds['sell_type'] )) {
   
   if ($conds['sell_type'] != "") {
    if($conds['sell_type'] != '0'){
    
     $this->db->where_not_in( 'bs_items.sell_type', $conds['sell_type'] ); 
    }
		...
		if ( isset( $conds['category_id'] )) {
			if ($conds['category_id'] != "") {
				if($conds['category_id'] != '0'){
					...
					$this->db->where( 'category_id', $conds['category_id'] );	
				}
...
// 					$this->db->where( 'sell_type', $conds['sell_type'] );	
// 				}
...
// 		  }
		  ...
		  if( isset($conds['item_type']) ) {
			if( $conds['item_type'] != '' ) {
				$this->db->where( 'item_type =', $conds['item_type'] );
			}	
		  }
		...
		if(isset($conds['mileage'])) {
		    if ($conds['mileage'] != "" || $conds['mileage'] != 0) {
			 ...
			 //   }else{
			 //       $this->db->where( 'mileage >= ', 40000 );	
			 //   }
		}
        if(isset($conds['max_mileage'])){
            if ($conds['max_mileage'] != "" || $conds['max_mileage'] != 0) {
                $this->db->where( 'mileage <= ', $conds['max_mileage'] );
            }
        }
...
// 	    if ( isset( $conds['item_type'] )) {
// 			if ($conds['item_type'] != "") {
// 				if($conds['item_type'] != '0'){
				
// 					$this->db->where( 'sell_type', $conds['item_type'] );	
// 				}
...
// 		}
		...
		if ( isset( $conds['service_area'] )) {
		    if($conds['category_id'] == SERVICES)
			    $this->db->where( 'service_area', $conds['service_area'] );
		}
		...
		if(isset($_GET['is_request_count'])){
          	if($_GET['is_request_count']){
          	    $queryCount = $this->db->get_compiled_select();
          	    $count = $this->db->query('( '. $query1 . ' ) UNION DISTINCT (' . $queryCount .') ')->num_rows();
          	    if($count){
      		    echo json_encode(['status'=>1,"message"=>"Success","response"=>["count"=>$count]]);
          	    }else{
          	        echo json_encode(['status'=>0,"message"=>"Failed","response"=>[]]);
          	    }
      		    exit();
          	}
  		}
