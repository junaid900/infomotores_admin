 ...
 * Items Controller
 */
class Accepted extends BE_Controller {
	...
	function __construct() {
		...
		parent::__construct( MODULE_CONTROL, 'ACCEPTED' );
	...
	function index() {
		...
		$conds['status'] = 1;
		$conds['expiry_date'] = date('Y-m-d H:m:s');
		
// 		print_r("2022-12-19 16:41:00" <= date('Y-m-d H:m:s'));exit;
		
		// get rows count
		
		$this->data['rows_count'] = $this->Accepteds->count_all_by( $conds);

		// get categories
		$this->data['accepted'] = $this->Accepteds->get_all_by( $conds , $this->pag['per_page'], $this->uri->segment( 4 ) );
	...
	function save( $id = false ) {
			...
			if ( $this->has_data( 'item_is_published' )) {
				...
				if ( $this->has_data( 'expiry_date' )) {
				$data['expiry_date'] = $this->get_data( 'expiry_date' );
			} else{
			    return;
			}
...
// 			} 
			...
			//save item
			if ( ! $this->Accepteds->save( $data, $id )) {
		...
		$id = ( !$id )? $data['id']: $id ;
		$title = $this->Accepteds->get_one($id)->title;
		//// Start - Send Noti /////
		if($data['status'] == 1) {
		...
		$user_ids = $this->Accepteds->get_one($id)->added_user_id;

		//for user name
		$user_id = $this->Accepteds->get_one($id)->added_user_id;
    ...
    //get all models when select manufacturer
	...
	*/
	function edit( $id ) 
	{
		...
		$this->data['action_title'] = get_msg( 'accepted_edit' );

		// load user
		$this->data['accepted'] = $this->Accepteds->get_one( $id );
