 ...
 * Items Controller
 */
class Expires extends BE_Controller {
	...
	function __construct() {
		...
		parent::__construct( MODULE_CONTROL, 'EXPIRES' );
...
// 		print_r('2022-12-18 15:52:00' <= date('Y-m-d H:m:s'));
// 		exit;
		
		// get rows count
		$this->data['rows_count'] = $this->Expire->count_all_by( $conds );
	

// 		// get categories
 		$this->data['expires'] = $this->Expire->get_all_by( $conds , $this->pag['per_page'], $this->uri->segment( 4 ) );
...
// 			
		...
		$id = ( !$id )? $data['id']: $id ;
		$title = $this->Expire->get_one($id)->title;
		//// Start - Send Noti /////
		if($data['status'] == 1) {
		...
		$user_ids = $this->Expire->get_one($id)->added_user_id;

		//for user name
		$user_id = $this->Expire->get_one($id)->added_user_id;
	...
	*/
	function edit( $id ) 
	{
		...
		// load user
		$this->data['expire'] = $this->Expire->get_one( $id );
