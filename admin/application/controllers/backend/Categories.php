...
//        echo "here2";
        ...
        // get categories
        // $this->data['items'] = $this->Category->get_all_by( $conds , $this->pag['per_page'], $this->uri->segment( 4 ) );
//        print_r($this->data);
//        exit();

        $this->data['categories'] = $this->db->get("bs_category")->result();
        // load index logic
        // load index logic
        parent::index();
    }
    public function save_data($type = "add",$param1 = ""){
        // print_r($_POST);
        // print_r($_FILES);
        if($type == "add"){
            $saveData = array();
            if (!empty($_FILES['mimage']['name'])) {
                ...
                $path_to_file = 'uploads/categories/' . $file_name;
                move_uploaded_file($_FILES['mimage']['tmp_name'], $path_to_file);
                $saveData['category_image'] = $path_to_file;
                // print_r($saveData);
            }
            $saveData["category_name"] = $_POST["title"];
            $res = $this->db->insert("bs_category",$saveData);
            if($res){
        ...
        if($type == "update"){
            ...
            if (!empty($_FILES['mimage']['name'])) {
                ...
                // print_r($saveData);
                 ...
                 $saveData['category_image'] = $_POST['pre_image'];
            }
            $saveData["category_name"] = $_POST["title"];
            $res = $this->db->update("bs_category",$saveData,["category_id"=>$param1]);
            if($res){
                redirect(base_url()."index.php/admin/categories");
            }else{
               echo "<script>alert('cannot insert')</script>";
    ...
    function add() {
        ...
        // breadcrumb urls
        $this->data['action_title'] = get_msg( 'add_category' );
        $this->data['type'] = "add";
    ...
    function edit( $id )
    {
        ...
        // load user
        $this->data['type'] = 'edit';
        $this->data['item'] = $this->db->get_where("bs_category",["category_id"=>$id])->row_array();
    ...
    function delete( $id )
    {
        ...
        $type = "item";
        $res = $this->db->delete("bs_category",["category_id" => $id]);
    ...
    function duplicate_item_save( $id ) {
        ...
        $items = $this->db->get_where("bs_category",["category_id"=>$id])->first_row();
        $added_date = date("Y-m-d H:i:s");
        $itm_data = array(
            'category_name' => $items->category_name,
            'category_image' => $items->category_image,
             'status' => $items->status,
        ...
        //save item
        if ( ! $this->db->insert('bs_category' ,$itm_data )) {
            // if there is an error in inserting user data,
