...
class Category extends PS_Model {
	...
	{
	   // echo "controller";
        parent::__construct( 'bs_category', 'category_id', 'cat_' );
	}
    function md(){
        // echo "ind";
    }
...
//		}
		...
		if ( isset( $conds['order_by_field'] )) {
			...
			$this->db->order_by( 'bs_category.'.$order_by_field, $order_by_type);
		} else {
		...
		// title condition
		if ( isset( $conds['category_name'] )) {
			$this->db->where( 'category_name', $conds['category_name'] );
		}
